...
export class AuthService  {
    ...
    async loginWithToken(input: AuthInputModel, context: { ip: string, userAgent: string }): Promise<Result<{ accessToken: string, refreshToken: string } | null>> {
        ...
        const [accessToken, refreshToken] = await Promise.all([
            this.jwtService.createAccessToken(userId, deviceId),
            this.jwtService.createRefreshToken(userId, deviceId)
        ]);
        const {iat, exp} = await this.jwtService.decodeToken(refreshToken)
    ...
    async checkCredentials(input: AuthInputModel): Promise<Result<WithId<UserDbTypes> | null>> {
        ...
        const validPassword = await this.bcryptService.checkPassword(input.password, user.passwordHash);
    ...
    async  refreshTokens(oldRefreshToken: string): Promise<Result<{ newAccessToken: string, newRefreshToken: string } | null>> {
        const payload = await this.jwtService.getPayloadFromToken(oldRefreshToken);
        ...
        const refreshPayload = await this.jwtService.decodeToken(newRefreshToken)
        await this.sessionsRepository.updateSessionActivity(deviceId, new Date(refreshPayload.iat * 1000));
    ...
    async revokeRefreshToken(refreshToken: string): Promise<Result<null>> {
        const payload  = await this.jwtService.getPayloadFromToken(refreshToken);
        if (!payload ) return ResultObject.Unauthorized('User is not authorized', [
            { field: 'RefreshToken', message: 'Invalid Token' }
        ]);
